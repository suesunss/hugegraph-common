...
public abstract class RestClient {
    ...
    private static final Logger LOG = Log.logger(RestClient.class);
    ...
    private Response request(Callable<Response> method) {
        Object property = this.config.getProperty(CONNECTION_MANAGER);
        if (property != null) {
            PoolingHttpClientConnectionManager pcm =
                    (PoolingHttpClientConnectionManager) property;
            LOG.debug("connection pool：{}", pcm.getTotalStats());
        }
        try {
            return method.call();
        } catch (Exception e) {
    ...
    private static class ConfigBuilder {
        ...
        public ConfigBuilder config(int timeout, int maxTotal,
            ...
            config.property(CONNECTION_MANAGER, pcm);
